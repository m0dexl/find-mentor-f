...
export class AuthService {
  ...
  public get currentValue(): User | null {
    //kullanıcının mevcut durumunu okuma işlemi
  ...
  public async register(request: RegisterRequest): Promise<ResponseStatus> {
    ...
    if (status == ResponseStatus.Ok) {
      // İşlem durumu 'Ok' olduğunda (kayıt başarılı ise) bu blok çalışır.
  ...
  public async refreshToken(): Promise<boolean> {
    ...
    if (refreshTokenResponse!.status == ResponseStatus.Ok) {
      // Oturum yenileme işleminin sonucunu kontrol et
  ...
  private setToken(token: TokenResponse | null) {
    // Oturum bilgilerini saklama işlemi başlar.
    if (token != null) {
      // Eğer gelen oturum bilgileri (token) null değilse, bu bloğa girilir.
      ...
      ); // Belirtecin son kullanma süresini sakla
      sessionStorage.setItem(
        'refresh_token',
        JSON.stringify(token.refreshToken)
      ); // Yenileme belirtecini tarayıcı oturumu içinde sakla
    }
  }

  async logOut() {
    // Oturum kapatma işlemi başlar.
